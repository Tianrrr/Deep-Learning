...
def load_vocab(base_dir):
    ...
    with open(vocab_file, 'rb') as f:   #'rb'以二进制的方式读取文件
        vocab = pickle.load(f)         # 从文件中将二进制数据转换成python数据 
    return vocab
...
def create_dummy_data(base_dir, sample_size=3, max_len=20, min_len=5):
    ...
    data = random.sample(train_data1, sample_size) + random.sample(train_data0, sample_size)  #data = A+B， 加号代表两个列表拼起来
    return data
