...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_minus_mean = x - sample_mean.reshape(1,C,1,1) 
    
        sq = x_minus_mean ** 2
    
        var = 1. / C * np.sum(sq, axis=(0,2,3))

        sqrtvar = np.sqrt(var + eps)

        ivar = 1. / sqrtvar

        x_norm = x_minus_mean * ivar.reshape(1,C,1,1)

        gammax = gamma.reshape(1,C,1,1) * x_norm

        out = gammax + beta.reshape(1,C,1,1)

        running_var = momentum * running_var + (1 - momentum) * var.reshape(1,C,1,1)
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean.reshape(1,C,1,1)
