...
class FacialKeypointsDataset(BaseDataset):
    ...
    def _get_image(idx, key_pts_frame):
        img_str = key_pts_frame.loc[idx]['Image']  #拿到的图片是一个str '1 2 3 4 5'     用split将str按空格分割后转换成int 然后在抓换成numpy，再reshape
        img = np.array([
            int(item) for item in img_str.split()
        ]).reshape((96, 96))
        # return np.expand_dims(img, axis=2).astype(np.uint8) #shape: 96,96,1
        return np.array([img,img,img]).astype(np.uint8).transpose(1,2,0) #shape: 3,96,96
    ...
    def _get_keypoints(idx, key_pts_frame, shape=(15, 2)):
        ...
        key_pts = key_pts_frame.iloc[idx][keypoint_cols].values.reshape(shape)   # 15,2的二维数组
        key_pts = (key_pts.astype(np.float) - 48.0) / 48.0    #归一化？？
