...
class Encoder(nn.Module):
    ...
    def __init__(self, hparams, input_size=28 * 28, latent_dim=20):
        ...
        self.encoder = nn.Sequential(nn.Linear(self.input_size, self.hparams['n_hidden'][0]),
                            nn.Tanh(),
                            nn.Linear(self.hparams['n_hidden'][0], self.hparams['n_hidden'][1]),
                            nn.Tanh(),
                            nn.Linear(self.hparams['n_hidden'][1], self.hparams['n_hidden'][2]),
                            nn.Tanh(),
                            nn.Linear(self.hparams['n_hidden'][2], self.hparams['latent_size']),
                            nn.Tanh()
                            )
...
class Decoder(nn.Module):
    ...
    def __init__(self, hparams, latent_dim=20, output_size=28 * 28):
        ...
        self.decoder = nn.Sequential(nn.Linear(self.hparams['latent_size'], self.hparams['n_hidden'][2]),
                            nn.Tanh(),
...
class Autoencoder(pl.LightningModule):
    ...
    def test(self):
        ...
        for idx, batch in enumerate(loader, 0):
            ...
            X = X.to(self.device)
            flattened_X = X.view(X.shape[0], -1)
            feature = self.encoder(flattened_X)
            Features.append(
                feature.view(-1, 10).cpu().detach().numpy())
        print(Features)
...
class Classifier(pl.LightningModule):
    ...
    def __init__(self, hparams, encoder, train_set=None, val_set=None, test_set=None):
        ...
        self.model = nn.Sequential(nn.Linear(self.hparams['hidden_size'], self.hparams['hidden_size']),   
                                    nn.Tanh(),
    ...
    def configure_optimizers(self):
        ...
        optim = torch.optim.Adam(self.model.parameters(), lr=self.hparams['learning_rate'], weight_decay=1e-3)
