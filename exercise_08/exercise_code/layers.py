...
def batchnorm_backward(dout, cache):
    ...
    dx_norm = dout * gamma
    dsigma_2 = np.sum((-1/2) * dx_norm *  x_minus_mean * (ivar ** 3), axis=0)

    dmean = -np.sum(dx_norm * ivar,axis=0) - dsigma_2 * np.sum(2 * x_minus_mean, axis=0) / N


    dx = dx_norm * ivar + (dmean / N) + dsigma_2 * 2 * x_minus_mean / N
