...
class SGDMomentum(object):
    ...
    def __init__(self, model, loss_func, learning_rate=1e-4, **kwargs):
        ...
        self.lr = learning_rate
        self.grads = None
        self.optim_config = kwargs.pop('optim_config', {})   #pop()根据键将字典中指定的键值对删除，同时可以将删除的值赋值给变量，若没有，返回默认值{}。
        self._reset()

    def _reset(self):
        self.optim_configs = {}
        for p in self.model.params:   # 遍历的是params的key （W1,b1,W2,b2...)
    ...
    def _update(self, w, dw, config, lr):
        ...
        if config is None:
            config = {}
        config.setdefault('momentum', 0.9)   #与get类似，如果config为空字典，则不存在键momentum,则返回默认值0.9，并写入字典中
        v = config.get('velocity', np.zeros_like(w))  # 如果没有设置 v，输出默认的值  与w形状相同的0矩阵  但不写入字典中
        next_w = None
